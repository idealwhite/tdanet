...
class Contrast(BaseModel):
    ...
    def name(self):
        return "Contrast Image Completion"
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['kl_rec','l1_rec','gan_rec','ad_l2_rec','word_rec','sentence_rec','dis_img_rec','contrast_rec',
                           'kl_g',  'l1_g', 'gan_g',   'ad_l2_g',  'word_g',  'sentence_g', 'dis_img', 'contrast_g']
        self.log_names = []
        ...
        self.net_E = network.define_contract_e(ngf=32, z_nc=256, img_f=256, layers=5, norm='none', activation='LeakyReLU',
                          init_type='orthogonal', gpu_ids=opt.gpu_ids, image_dim=256, text_dim=256, multi_peak=False, pool_attention=self.max_pool)
        self.net_G = network.define_constrast_g(output_nc=3, ngf=32, z_nc=256, img_f=256, L=0, layers=5, output_scale=opt.output_scale,
    ...
    def test(self, mark=None):
        ...
        # encoder process
        distribution, v, h_text = self.net_E(
        ...
        scale_mask = task.scale_img(self.mask, size=[v[2].size(2), v[2].size(3)])

        # decoder process
        for i in range(self.opt.nsampling):
            z = q_distribution.sample()

            self.img_g, attn = self.net_G(z, h_text, f_e=v[2], mask=scale_mask.chunk(3, dim=1)[0])
    ...
    def get_G_inputs(self, p_distribution, q_distribution, f, h_text):
        """Process the encoder feature and distributions for generation network, combine two dataflow when implement."""
        h = torch.cat([h_text.chunk(2)[0], f[-1].chunk(2)[1]], dim=0)
        v_l = torch.cat([f[2].chunk(2)[0], f[2].chunk(2)[0]], dim=0)
        scale_mask = task.scale_img(self.mask, size=[v_l.size(2), v_l.size(3)])
    ...
    def forward(self):
        ...
        # encoder process
        distribution_factors, v, h_text = self.net_E(
