...
class TextualPluralistic(BaseModel):
    ...
    def test(self):
        ...
        # encoder process
        distribution, f = self.net_E(self.img_m, self.sentence_embedding)
        ...
        for i in range(self.opt.nsampling):
            ...
            weighted_word_embedding_g = self.word_attention( \
                f_e, self.word_embeddings, mask=self.text_mask, image_mask=mask, inverse_attention=True)

            self.img_g, attn = self.net_G(z, f_m, f_e, weighted_word_embedding_g, mask)
    ...
    def forward(self):
        ...
        # encoder process
        distribution_factors, f = self.net_E(self.img_m, self.sentence_embedding, self.img_c)
